...
class Ion_auth_model extends CI_Model
{
	 ...
	 * @author Mathew
	 **/
	public function register($usid,$identity, $password, $email, $additional_data = array(), $groups = array())
	{
	...
	$email_activation = $this->config->item('email_activation', 'ion_auth');
		...
		//$user_data = array_merge($this->_filter_data($this->tables['users'], $additional_data), $data);
               ...
               // $profile_data = $this->_filter_data($this->tables['profiles'], $additional_data);
		...
		$id = $this->db->insert_id();
                ...
                $this->add_to_user_profile($additional_data);
                  //$this->db->insert($this->tables['profiles'], $additional_data);
...
/**
	 ...
	 * @author Mathew
	 **/
	public function reg($uid, $identity, $password,$email, $school_id, $department_id, $program_type,  $additional = array(), $dataarr = array(), $groups = array())
	{
		$this->trigger_events('pre_register');
		...
		elseif ( !$this->config->item('admin_group', 'ion_auth') && empty($groups) )
		{
			$this->set_error('account_creation_missing_default_group');
			return FALSE;
		}
		...
		// check if the default set in config exists in database
		$query = $this->db->get_where($this->tables['groups'],array('name' => $this->config->item('admin_group', 'ion_auth')),3)->row();
		if( !isset($query->id) && empty($groups) )
		{
			$this->set_error('account_creation_invalid_default_group');
			return FALSE;
		...
		$id = $this->db->insert_id();
                ...
                $this->add_admin($additional);
                  $this->add_to_user_profile($dataarr);
                  //$this->db->insert($this->tables['profiles'], $additional_data);
		// add in groups array if it doesn't exists and stop adding into default group if default group ids are set
		if( isset($default_group->id) && empty($groups) )
		{
...
/**
	 ...
	 * @author Mathew
	 **/
	public function adminregstud($uid, $identity, $password,$email, $school_id, $department_id, $program_type,  $additional = array(), $dataarr = array(), $groups)
	{
		$this->trigger_events('pre_register');
		...
		// check if the default set in config exists in database
		$query = $this->db->get_where($this->tables['groups'],array('name' => $this->config->item('student_group', 'ion_auth')),3)->row();
		if( !isset($query->id) && empty($groups) )
		{
			$this->set_error('account_creation_invalid_default_group');
			return FALSE;
		...
		$id = $this->db->insert_id();
                ...
                $this->admin_add_student($additional);
                  $this->add_to_user_profile($dataarr);
                  //$this->db->insert($this->tables['profiles'], $additional_data);
		// add in groups array if it doesn't exists and stop adding into default group if default group ids are set
		if( isset($default_group->id) && empty($groups) )
	 ...
	 * @author Mathew
	 **/
	public function adminreglecturer($uid, $identity, $password,$email, $school_id, $department_id, $program_type,  $additional = array(), $dataarr = array(), $groups = array())
	{
		$this->trigger_events('pre_register');
		...
		// check if the default set in config exists in database
		$query = $this->db->get_where($this->tables['groups'],array('name' => $this->config->item('lecturer_group', 'ion_auth')),3)->row();
		if( !isset($query->id) && empty($groups) )
		{
			$this->set_error('account_creation_invalid_default_group');
			return FALSE;
		...
		$id = $this->db->insert_id();
                ...
                $this->admin_add_lecturer($additional);
                  $this->add_to_user_profile($dataarr);
                  //$this->db->insert($this->tables['profiles'], $additional_data);
		// add in groups array if it doesn't exists and stop adding into default group if default group ids are set
		if( isset($default_group->id) && empty($groups) )
	 ...
	 * @author Mathew
	 **/
	public function adminreghod($uid, $identity, $password,$email, $school_id, $department_id, $program_type,  $additional = array(), $dataarr = array(), $groups = array())
	{
		$this->trigger_events('pre_register');
		...
		// check if the default set in config exists in database
		$query = $this->db->get_where($this->tables['groups'],array('name' => $this->config->item('hod_group', 'ion_auth')),3)->row();
		if( !isset($query->id) && empty($groups) )
		{
			$this->set_error('account_creation_invalid_default_group');
			return FALSE;
...
/**
	 ...
	 * @author Mathew
	 **/
	public function adminreglib($uid, $identity, $password,$email, $school_id, $program_type,  $additional = array(), $dataarr = array(), $groups = array())
	{
		$this->trigger_events('pre_register');
		...
		// check if the default set in config exists in database
		$query = $this->db->get_where($this->tables['groups'],array('name' => $this->config->item('lib_group', 'ion_auth')),3)->row();
		if( !isset($query->id) && empty($groups) )
		{
			$this->set_error('account_creation_invalid_default_group');
			return FALSE;
	 ...
	 * @author Mathew
	 **/
	public function adminregsrc($uid, $identity, $password,$email, $school_id, $program_type,  $additional = array(), $dataarr = array(), $groups = array())
	{
		$this->trigger_events('pre_register');
		...
		// check if the default set in config exists in database
		$query = $this->db->get_where($this->tables['groups'],array('name' => $this->config->item('src_group', 'ion_auth')),3)->row();
		if( !isset($query->id) && empty($groups) )
		{
			$this->set_error('account_creation_invalid_default_group');
			return FALSE;
		...
		return (isset($id)) ? $id : FALSE;
           ...
           public function admin_add_student($data)
        {
           $this->db->insert($this->tables['students'], $data); 
           	return TRUE;
        }
    public function admin_add_src($data)
        {
           $this->db->insert($this->tables['src'], $data); 
           	return TRUE;
        }
            public function admin_add_lib($data)
        {
           $this->db->insert($this->tables['lib'], $data); 
           	return TRUE;
        }
               public function admin_add_lecturer($data)
        {
           $this->db->insert($this->tables['lecturer'], $data); 
           	return TRUE;
        }
                  public function admin_add_hod($data)
        {
           $this->db->insert($this->tables['hod'], $data); 
           	return TRUE;
        }
       public function add_admin($data)
        {
           $this->db->insert($this->tables['admins'], $data); 
           	return TRUE;
        } 
        
        public function add_to_user_profile($data)
        {
           $this->db->insert($this->tables['profiles'], $data); 
           	return TRUE;
        }
	...
	public function login($identity, $password, $remember=FALSE)
	{
		...
		$query = $this->db->select($this->identity_column . ',username,uid, email, id, password, active, last_login')
		                  ->where($this->identity_column, $identity)
		                  ->limit(1)
		    			  ->order_by('id', 'desc')
		                  ->get($this->tables['users']);
		...
		if ($query->num_rows() === 1)
		{
			...
			if ($password === TRUE)
			{
				...
				//$this->update_online_status($user->id);
	...
	**/
	...
	public function update_last_login($id)
	{
		...
		$this->db->update($this->tables['users'], array('last_login' => time(),'status' => 1), array('id' => $id));
...
/**
	...
	public function deactivatestatus($id)
	{
		...
		$this->db->update($this->tables['users'], array('status' => 0), array('id' => $id));
